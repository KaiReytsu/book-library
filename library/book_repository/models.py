...
class Author(models.Model):
    ...
    author_photo = models.ImageField(
        verbose_name="Фотография автора", upload_to="author_img/", null=True, blank=True
    )
...
class Book(models.Model):
    ...
    book_image = models.ImageField(
        verbose_name="Обложка книги", upload_to="book_img/", null=True, blank=True
    )
    ...
    publication_language = models.ForeignKey(
        Language, verbose_name="Язык издания", max_length=20, on_delete=models.CASCADE
    )
...
class Reservation(models.Model):
    ...
    class Status(models.TextChoices):
        ...
        SPOILED = "испорчено", gettext_lazy("испорчено")

    user = models.ForeignKey(
        User, verbose_name="Имя пользователя", on_delete=models.CASCADE
    )
    book = models.ForeignKey(
        Book, verbose_name="Забронированная книга", on_delete=models.CASCADE
    )
    ...
    hash = models.CharField(
        verbose_name="Номер бронирования", default="", max_length=5, unique=True
    )

    def save(self, *args, **kwargs):
        self.hash = hashlib.md5(
            (
                str(self.id) + str(self.user) + str(self.book) + str(self.date_of_issue)
            ).encode("utf-8")
        ).hexdigest()[-5:]
